...
class GoPro:
			...
			if path == "":
				if GPR == True:
					if path.endswith("JPG"):
						opener.urlretrieve(self.getMedia().replace("JPG","GPR"), self.getMediaInfo("folder")+"-"+self.getMediaInfo("file"))
					else:
						print("Media is not a JPG.")
				print("filename: " + self.getMediaInfo("file") + "\nsize: " + self.getMediaInfo("size"))
				opener.urlretrieve(self.getMedia(), self.getMediaInfo("folder")+"-"+self.getMediaInfo("file"))
			else:
				if GPR == True:
					if path.endswith("JPG"):
						if custom_filename == "":
							opener.urlretrieve(self.getMedia().replace("JPG","GPR"), self.getMediaInfo("folder")+"-"+self.getMediaInfo("file").replace("JPG","GPR"))
						else:
							opener.urlretrieve(self.getMedia().replace("JPG","GPR"), custom_filename)
					else:
						print("Media is not a JPG.")
				...
				opener.urlretrieve(path, filename)
		else:
	...
	def downloadMedia(self, folder, file, custom_filename=""):
		if self.IsRecording() == 0:
			...
			if custom_filename == "":
				...
				filename = custom_filename
			try:
				opener.urlretrieve("http://" + self.ip_addr + ":8080/videos/DCIM/" + folder + "/" + file, filename)
			except (HTTPError, URLError) as error:
	...
	def downloadLowRes(self, path="", custom_filename = ""):
		if self.IsRecording() == 0:
			if path == "":
				...
				if custom_filename == "":
					try:
						opener.urlretrieve(lowres_url, lowres_filename)
					except (HTTPError, URLError) as error:
						print("ERROR: " + str(error))
				else:
					try:
						opener.urlretrieve(lowres_url, custom_filename)
					except (HTTPError, URLError) as error:
						print("ERROR: " + str(error))
			else:
				lowres_url=""
	...
	def downloadLastSpherical(self):
		if self.IsRecording() == 0:
			print("filename: " + self.getMediaInfo("file") + "\nsize: " + self.getMediaInfo("size"))
			opener.urlretrieve(self.getMedia(), self.getMediaInfo("folder")+"-"+self.getMediaInfo("file"))
			opener.urlretrieve(self.getMediaFront(), self.getMediaInfoFront("folder")+"-"+self.getMediaInfoFront("file"))
		else:
			print("Not supported while recording or processing media.")
